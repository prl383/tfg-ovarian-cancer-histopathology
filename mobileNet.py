...
imágenes histopatológicas mediante Transfer Learning con MobileNetV2.

Autor: Patricia Rodríguez Lidueña
Entorno: Google Colab (pruebas iniciales) y servidor GPU dedicado (ejecución final)
...
# Log, gráficas, pesos y resumen JSON, todo junto para copiar a otra máquina
...
NOMBRE_MODELO = "MobileNetV2"  # usado en la tabla comparativa del Paso 16
...
# =============================================================================
# PASO 4: DEFINICIÓN DEL MODELO (MOBILENETV2 + TRANSFER LEARNING)
# =============================================================================
def crear_modelo(num_classes: int, dropout: float = 0.3) -> nn.Module:
    """
    MobileNetV2 preentrenada en ImageNet con Fine-Tuning parcial: se congela
    todo el backbone salvo su último bloque (features[-1]), que se reentrena
    junto con la nueva cabeza clasificadora.

    "dropout" lo busca Optuna como hiperparámetro más (Paso 12), tras
    detectar overfitting severo en una ejecución anterior sin él.
    """
    ...
    # Último bloque de "features" (ConvBNReLU final, 320 -> 1280 canales)
    for parametro in modelo.features[-1].parameters():
        parametro.requires_grad = True

    # Cabeza original: Sequential(Dropout, Linear(1280 -> 1000))
...
def ejecutar_5fold_cv(
    ...
    for numero_fold, (pos_train, pos_val) in enumerate(skf.split(indices_pool, labels_pool), start=1):
        ...
        if guardar_pesos:
            torch.save(modelo.state_dict(), os.path.join(CARPETA_MODELOS, f"mobilenetv2_{prefijo}_fold{numero_fold}.pth"))
...
def ejecutar_experimento_completo(nombre_experimento: str, dataset_entrenamiento, n_trials: int) -> dict:
    ...
    def objetivo_optuna(trial: optuna.Trial) -> float:
        hparams = {
            ...
            "dropout": trial.suggest_float("dropout", 0.2, 0.5),  # añadido tras overfitting severo en un run anterior
        }
    ...
    estudio_optuna = optuna.create_study(
        direction="maximize",
        study_name=f"mobilenetv2_{nombre_experimento}",
    ...
    torch.save(modelo_final.state_dict(), os.path.join(CARPETA_MODELOS, f"mobilenetv2_final_{nombre_experimento}.pth"))
...
# Copia en la raíz (donde app.py lo espera) y en resultados/modelos/
torch.save(resultado_ganador["modelo_final"].state_dict(), "mobilenetv2_final.pth")
torch.save(resultado_ganador["modelo_final"].state_dict(), os.path.join(CARPETA_MODELOS, "mobilenetv2_final.pth"))
print('Pesos del modelo ganador guardados en "mobilenetv2_final.pth" (raíz, para app.py) y en "resultados/modelos/".')
...
# activaciones de features[-1] (la capa entrenable en el Fine-Tuning).
class GradCAM:
...
modelo_gradcam.eval()
gradcam = GradCAM(modelo_gradcam, modelo_gradcam.features[-1])
