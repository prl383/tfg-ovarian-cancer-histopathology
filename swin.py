...
imágenes histopatológicas mediante Transfer Learning con Swin-Tiny.

Cuarto y último modelo del TFG: mismo pipeline exacto, adaptado en lo
específico de la arquitectura (crear_modelo() y la capa objetivo de
Grad-CAM). A diferencia de las tres CNN anteriores, Swin-Tiny produce
activaciones en formato (lote, alto, ancho, canales) en vez de (lote,
canales, alto, ancho), lo que obliga a adaptar también la clase GradCAM
(ver Paso 15). Ver mobileNet.py para comentarios más detallados de cada paso.
...
CARPETA_RESULTADOS = "resultados_swin"
...
NOMBRE_MODELO = "Swin-Tiny"  # usado en la tabla comparativa del Paso 16
...
# =============================================================================
# PASO 4: DEFINICIÓN DEL MODELO (SWIN-TINY + TRANSFER LEARNING)
# =============================================================================
def crear_modelo(num_classes: int, dropout: float = 0.3) -> nn.Module:
    """
    Swin-Tiny (Transformer de visión jerárquico) preentrenada en ImageNet
    con Fine-Tuning parcial: se congela todo el backbone salvo el último
    bloque Swin de su última etapa (features[-1][-1]), que se reentrena
    junto con la nueva cabeza clasificadora. Mismo criterio que en los tres
    modelos anteriores.

    "dropout" lo busca Optuna como hiperparámetro más (Paso 12).
    """
    weights = Swin_T_Weights.IMAGENET1K_V1
    modelo = swin_t(weights=weights)

    for parametro in modelo.parameters():
        parametro.requires_grad = False

    # features[-1] es la última etapa (2 bloques Swin); se descongela solo el último
    for parametro in modelo.features[-1][-1].parameters():
        parametro.requires_grad = True

    # Cabeza original: Linear(768 -> 1000)
    num_features = modelo.head.in_features
    modelo.head = nn.Sequential(
...
def ejecutar_5fold_cv(
    ...
    for numero_fold, (pos_train, pos_val) in enumerate(skf.split(indices_pool, labels_pool), start=1):
        ...
        if guardar_pesos:
            torch.save(modelo.state_dict(), os.path.join(CARPETA_MODELOS, f"swin_{prefijo}_fold{numero_fold}.pth"))
...
def ejecutar_experimento_completo(nombre_experimento: str, dataset_entrenamiento, n_trials: int) -> dict:
    ...
    estudio_optuna = optuna.create_study(
        direction="maximize",
        study_name=f"swin_{nombre_experimento}",
    ...
    torch.save(modelo_final.state_dict(), os.path.join(CARPETA_MODELOS, f"swin_final_{nombre_experimento}.pth"))
...
# Copia en la raíz y en CARPETA_MODELOS. Nombre no genérico ("swin_final.pth"):
# app.py todavía solo carga MobileNetV2, así que este archivo es de referencia/backup.
torch.save(resultado_ganador["modelo_final"].state_dict(), "swin_final.pth")
torch.save(resultado_ganador["modelo_final"].state_dict(), os.path.join(CARPETA_MODELOS, "swin_final.pth"))
print(f'Pesos del modelo ganador guardados en "swin_final.pth" (raíz) y en "{CARPETA_MODELOS}".')
...
#
# DIFERENCIA IMPORTANTE respecto a las 3 CNN anteriores: una CNN produce
# activaciones en formato (lote, canales, alto, ancho); Swin-Tiny, al ser un
# Transformer de visión, las produce en formato (lote, alto, ancho, canales)
# -con los canales al FINAL. Reutilizar la clase GradCAM tal cual (que asume
# los canales en la posición 1) no daría error, pero calcularía un mapa de
# calor incorrecto sin avisar. Por eso aquí se permutan las activaciones y
# los gradientes a (lote, canales, alto, ancho) nada más capturarlos, para
# reutilizar la misma matemática de generar_mapa() que en los otros modelos.
class GradCAM:
    """Implementación mínima de Grad-CAM para una capa convolucional/Swin dada."""
    ...
    def _guardar_activaciones(self, modulo, entrada, salida):
        # (lote, alto, ancho, canales) -> (lote, canales, alto, ancho)
        self.activaciones = salida.detach().permute(0, 3, 1, 2)

    def _guardar_gradientes(self, modulo, grad_entrada, grad_salida):
        self.gradientes = grad_salida[0].detach().permute(0, 3, 1, 2)
