...
imágenes histopatológicas mediante Transfer Learning con ResNet50.

Segundo modelo del TFG (tras MobileNetV2): mismo pipeline exacto,
...
# Carpeta propia (no "resultados/" a secas) para no sobrescribir la de
# MobileNetV2 al ejecutar este script en el mismo proyecto
CARPETA_RESULTADOS = "resultados_resnet50"
...
NOMBRE_MODELO = "ResNet50"  # usado en la tabla comparativa del Paso 16
...
# =============================================================================
# PASO 4: DEFINICIÓN DEL MODELO (RESNET50 + TRANSFER LEARNING)
# =============================================================================
def crear_modelo(num_classes: int, dropout: float = 0.3) -> nn.Module:
    """
    ResNet50 preentrenada en ImageNet con Fine-Tuning parcial: se congela
    todo el backbone salvo su último bloque residual (layer4[-1]), que se
    reentrena junto con la nueva cabeza clasificadora. Mismo criterio que
    en MobileNetV2.
    ...
    # layer4 tiene 3 bloques Bottleneck; se descongela solo el último
    for parametro in modelo.layer4[-1].parameters():
        parametro.requires_grad = True

    # Cabeza original: Linear(2048 -> 1000)
...
def ejecutar_5fold_cv(
    ...
    for numero_fold, (pos_train, pos_val) in enumerate(skf.split(indices_pool, labels_pool), start=1):
        ...
        if guardar_pesos:
            torch.save(modelo.state_dict(), os.path.join(CARPETA_MODELOS, f"resnet50_{prefijo}_fold{numero_fold}.pth"))
...
def ejecutar_experimento_completo(nombre_experimento: str, dataset_entrenamiento, n_trials: int) -> dict:
    ...
    estudio_optuna = optuna.create_study(
        direction="maximize",
        study_name=f"resnet50_{nombre_experimento}",
    ...
    torch.save(modelo_final.state_dict(), os.path.join(CARPETA_MODELOS, f"resnet50_final_{nombre_experimento}.pth"))
...
# Copia en la raíz y en CARPETA_MODELOS. Nombre no genérico ("resnet50_final.pth"):
# app.py todavía solo carga MobileNetV2, así que este archivo es de referencia/backup.
torch.save(resultado_ganador["modelo_final"].state_dict(), "resnet50_final.pth")
torch.save(resultado_ganador["modelo_final"].state_dict(), os.path.join(CARPETA_MODELOS, "resnet50_final.pth"))
print(f'Pesos del modelo ganador guardados en "resnet50_final.pth" (raíz) y en "{CARPETA_MODELOS}".')
...
# activaciones de layer4[-1] (la capa entrenable en el Fine-Tuning).
class GradCAM:
...
modelo_gradcam.eval()
gradcam = GradCAM(modelo_gradcam, modelo_gradcam.layer4[-1])
